  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
        <div>
          <h2 style={{ fontSize: 16, fontWeight: 600, color: '#1B2430' }}>Likelihood × Impact Risk Heatmap</h2>
          <p style={{ fontSize: 12.5, color: '#5B6672', marginTop: 2 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {[5, 4, 3, 2, 1].map((l) => (
              <div key={l} style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                <span style={{ width: 130, fontSize: 12, fontWeight: 500, color: '#5B6672', textAlign: 'right', paddingRight: 8 }}>
                  {LIKELIHOOD_NAMES[l]}
                </span>
            ...
            {/* X-Axis Labels */}
            <div style={{ display: 'flex', gap: 8, marginTop: 4, paddingLeft: 138 }}>
              {[1, 2, 3, 4, 5].map((imp) => (
                <div key={imp} style={{ flex: 1, textAlign: 'center', fontSize: 11.5, fontWeight: 500, color: '#5B6672' }}>
                  {IMPACT_NAMES[imp]}
                </div>
              ))}
            </div>
